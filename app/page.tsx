    ...
    <main className={`shell ${view === "home" ? "home-shell" : ""}`}>
      ...
      <section className="workspace">
        ...
        {view === "slos" && <section className="full-page slo-page">
          <CurriculumPageToolbar heading={allSLOsSelected ? <div className="eyebrow">SESSION LEARNING OBJECTIVES</div> : <nav className="slo-breadcrumbs" aria-label="SLO folders">
          ...
          <div className="lecture-list curriculum-card-list">{visibleSloLectures.map((lecture) => {
            ...
            return <CurriculumCard key={lecture.id} title={lecture.title} course={lecture.course} lecturer={lecture.lecturer} week={lecture.week} countLabel={`${lecture.slos.length} SLO${lecture.slos.length === 1 ? "" : "s"}`} primaryActionLabel={expanded ? "Hide SLOs" : "View SLOs"} onPrimaryAction={toggleExpanded} expanded={expanded}>
        ...
        {viewerLecture && <div className={`viewer-modal ${pdfPaneHidden ? "pdf-pane-hidden" : ""} ${lunaPaneHidden ? "luna-pane-hidden" : ""} ${pdfPaneHidden && lunaPaneHidden ? "both-panes-hidden" : ""}`} role="dialog" aria-modal="true" aria-label="Lecture slide viewer">
          <section className={`viewer-stage ${pdfPaneHidden ? "pane-hidden" : ""}`}>
            ...
            <div className="viewer-slide-workspace">
              ...
              <section className="pdf-note-panel">
                <header><label htmlFor="pdf-slide-note">Notes · PDF page {selectedPage}</label><small aria-live="polite">{noteSaveStatus === "saving" ? "Saving…" : "Saved automatically"}</small></header>
                <textarea id="pdf-slide-note" aria-label={`Notes for PDF page ${selectedPage}`} value={noteDraft} onChange={(event) => setNoteDraft(event.target.value)} />
              </section>
            </div>
            ...
            <div className="viewer-conversation">
              ...
              <div className="conversation-dock">
                {viewerContext && <section className="viewer-context-drawer">
                  <header><strong>{viewerContext === "marks" ? "Marked slides" : "Flagged SLOs"}</strong><button aria-label="Close study context" onClick={() => setViewerContext(null)}>×</button></header>
