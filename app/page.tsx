  ...
  return <main className="shell simplified-shell"><section className="workspace">
    ...
    {viewerLecture && <div className="viewer-modal viewer-pdf-workspace viewer-canvas-controls" role="dialog" aria-modal="true" aria-label={viewerLecture.title}>
      <section className="viewer-stage">
        ...
        <div className="viewer-canvas-actions"><div className="viewer-zoom"><button aria-label="Zoom out" disabled={pdfZoom <= .6} onClick={() => setPdfZoom((current) => Math.max(.6, Number((current - .1).toFixed(1))))}>−</button><button onClick={() => setPdfZoom(1)}>Fit {Math.round(pdfZoom * 100)}%</button><button aria-label="Zoom in" disabled={pdfZoom >= 4} onClick={() => setPdfZoom((current) => Math.min(4, Number((current + .1).toFixed(1))))}>+</button></div><button className={tocOpen ? "active" : ""} onClick={() => setTocOpen((current) => !current)}>Contents</button><button className={penEnabled ? "active" : ""} onClick={() => setPenEnabled((current) => !current)}>Pen</button><button className={currentSlideIsMarked ? "active" : ""} onClick={() => void toggleCurrentSlideMark()}>{currentSlideIsMarked ? "Marked" : "Mark"}</button><button className="viewer-delete-lecture" onClick={() => void removeCurrentLecture()}>Delete</button><button onClick={() => setViewerLectureId("")}>Close</button></div>
