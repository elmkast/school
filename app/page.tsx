...
export default function Home() {
  ...
  const visibleSloLectures = useMemo(() => lectures.filter((lecture) => lecture.slos.length > 0).filter((lecture) => sloCourseFilter === "all" || lecture.course === sloCourseFilter).filter((lecture) => sloInstructorFilter === "all" || lecture.lecturer === sloInstructorFilter).filter((lecture) => sloWeekFilter === "all" || (sloWeekFilter === "unassigned" ? lecture.week === null : lecture.week === Number(sloWeekFilter))).filter((lecture) => sloViewFilter === "all" || lecture.flaggedSLOs.length > 0).sort((a, b) => sloSort === "name-asc" ? compareText(a.title, b.title) : compareLectureWeeks(a.week, b.week) || compareText(a.title, b.title)), [lectures, sloCourseFilter, sloInstructorFilter, sloSort, sloViewFilter, sloWeekFilter]);
  ...
  return <main className="shell simplified-shell"><section className="workspace">
    ...
    {view === "slos" && <section className="full-page slo-page slo-live-page"><SloWorkspace lectures={visibleSloLectures} courses={searchCourseOptions} instructors={lecturerOptions} courseFilter={sloCourseFilter} weekFilter={sloWeekFilter} instructorFilter={sloInstructorFilter} viewFilter={sloViewFilter} sort={sloSort} onCourseFilter={setSloCourseFilter} onWeekFilter={setSloWeekFilter} onInstructorFilter={setSloInstructorFilter} onViewFilter={setSloViewFilter} onSort={setSloSort} onExport={openSloExport} onToggleFlag={(lectureId, index) => void toggleSloFlag(lectureId, index)} onReparse={openSloReparse} onOpenLecture={openLecture}/></section>}
