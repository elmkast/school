...
export default function Home() {
  ...
  function acknowledgeImportDuplicate(id: string) {
    setUploadQueue((current) => current.map((job) => job.id === id ? { ...job, duplicateOverride: true } : job));
  }
  ...
  async function finalizeImports() {
    ...
    const stagedLectures = readyJobs.map((job) => job.lecture);
    const unresolvedQuality = readyJobs.find((job) => {
      const report = assessImportQuality({ lecture: job.lecture, fileName: job.name, aiFailed: job.aiFailed, library: lectures, batch: stagedLectures });
      return report.blocksFinalization || (report.duplicates.length > 0 && !job.duplicateOverride);
    });
    if (unresolvedQuality) { setNotice("Resolve the import checks before finalizing."); return; }
  ...
  return <main className="shell simplified-shell"><section className="workspace">
    ...
    {cloudSession && !cloudHasData && localReady && <section className="cloud-migration-banner"><div><small>ONE-TIME CLOUD SETUP</small><strong>Move this device’s lectures into your private account</strong><p>This copies lecture PDFs, SLOs, marks, and annotations. The originals remain on this computer.</p>{migrationProgress && <span>{migrationProgress.completed} of {migrationProgress.total}: {migrationProgress.label}</span>}</div><button disabled={migrationRunning} onClick={() => void migrateThisDevice()}>{migrationRunning ? "Migrating…" : "Migrate this device"}</button></section>}
    {uploadReviewOpen && <LectureImportReview jobs={uploadQueue} library={lectures} courses={searchCourseOptions} instructors={lecturerOptions} finalizing={uploadFinalizing} onUpdate={updateImportDraft} onRemove={removeImportJob} onAcknowledgeDuplicate={acknowledgeImportDuplicate} onAddMore={() => fileInput.current?.click()} onClose={() => setUploadReviewOpen(false)} onFinalize={() => void finalizeImports()}/>}
