...
import { deleteLecture, getLectureFile, loadCloudLibrary, loadLectures, migrateLocalLibraryToCloud, normalizeLecture, saveLecture, saveLectures, setCloudUser, type InkStroke, type Lecture, type MigrationProgress, type Slide } from "../lib/lecture-store";
...
import { LECTURE_WEEK_OPTIONS, compareLectureWeeks, compareText, lectureWeekLabel, lecturerFolderLabel } from "../lib/curriculum";
...
function mergeAiLectureBrief(base: Lecture, response: unknown, sourceSlides: Slide[], destination: ImportDestination) {
  ...
  const normalized = normalizeLecture({ ...base, ...accepted, slides, notes: {}, markups: {}, markedSlides: [], flaggedSLOs: [], academicYear: destination.academicYear, course: destination.course ?? accepted.course ?? base.course, lecturer: destination.lecturer ?? accepted.lecturer ?? base.lecturer, favorite: false });
...
const uploadStatusLabel: Record<UploadStatus, string> = { queued: "Waiting", extracting: "Extracting text", analyzing: "Structuring with Luna", saving: "Saving", done: "Complete", error: "Failed" };
...
export default function Home() {
  ...
  const visibleSloLectures = useMemo(() => lectures.filter((lecture) => lecture.slos.length > 0).filter((lecture) => sloWeekFilter === "all" || (sloWeekFilter === "flagged" ? lecture.flaggedSLOs.length > 0 : sloWeekFilter === "unassigned" ? lecture.week === null : lecture.week === Number(sloWeekFilter))).sort((a, b) => sloSort === "name-asc" ? compareText(a.title, b.title) : compareLectureWeeks(a.week, b.week) || compareText(a.title, b.title)), [lectures, sloSort, sloWeekFilter]);
  ...
  function openSearchResult(result: SearchResult) {
    ...
    else if (result.kind === "slo") { setView("slos"); setExpandedSloLectureIds(new Set([result.lecture.id])); }
  ...
  async function toggleSloFlag(lectureId: string, sloIndex: number) {
    ...
    setLectures((current) => current.map((item) => item.id === lectureId ? updated : item)); await saveLecture(updated); setNotice(wasFlagged ? "SLO removed from flagged SLOs." : "SLO flagged for review.");
  }
  ...
  async function acceptSloReparse() {
    ...
    const updated = { ...sloReparseLecture, slos, flaggedSLOs: slos.flatMap((slo, index) => flaggedTexts.has(slo) ? [index] : []) };
  ...
  async function importLecture(file: File, destination: ImportDestination, onStage: (status: UploadStatus) => void) {
    ...
    try {
      ...
      const lecture: Lecture = { id: crypto.randomUUID(), title, lecturer: "Lecturer not detected", week: null, course: "Unsorted", academicYear: destination.academicYear, favorite: false, pages: pageCount, slos: detectSLOs(slides), outline: [], summary: `Imported ${pageCount} slides.`, slides, notes: {}, markups: {}, markedSlides: [], flaggedSLOs: [], fileName: file.name, createdAt: new Date().toISOString() };
      ...
      onStage("saving"); const savedLecture = await saveLecture(lecture, file); setLectures((current) => [savedLecture, ...current]); clearUploadDiagnosticCheckpoint(); if (aiFailed) setNotice(`${file.name} imported with local extraction because Luna was unavailable.`);
  ...
  function updateUploadJob(id: string, changes: Partial<UploadJob>) { setUploadQueue((current) => current.map((job) => job.id === id ? { ...job, ...changes } : job)); }
  async function runUploadQueue() {
    if (uploadRunnerActive.current) return; uploadRunnerActive.current = true; setUploading(true);
    try { while (pendingUploads.current.length) { const job = pendingUploads.current.shift(); if (!job) continue; try { await importLecture(job.file, job.destination, (status) => updateUploadJob(job.id, { status })); updateUploadJob(job.id, { status: "done" }); } catch (error) { updateUploadJob(job.id, { status: "error", error: error instanceof Error ? error.message : "Import failed" }); } await new Promise<void>((resolve) => window.setTimeout(resolve, 50)); } }
    finally { uploadRunnerActive.current = false; setUploading(false); setNotice("Lecture import queue finished."); if (pendingUploads.current.length) void runUploadQueue(); }
  }
  ...
  function enqueueFiles(fileList: FileList | File[]) {
    ...
    const entries = files.map((file) => {
      ...
      return {
        ...
        display: { id, name: file.name, destinationLabel: destination.label, status: "queued" as const },
      };
    });
  ...
  return <main className="shell simplified-shell"><section className="workspace">
    ...
    {queueVisible && uploadQueue.length > 0 && <aside className="upload-queue"><header><div><small>IMPORT QUEUE</small><strong>{finishedUploads} of {uploadQueue.length} finished</strong></div><button onClick={() => setQueueVisible(false)}><AppIcon name="x"/></button></header><div className="queue-jobs">{uploadQueue.map((job) => <div className={`queue-job ${job.status}`} key={job.id}><span className="queue-indicator"/><div><strong>{job.name}</strong><small>{uploadStatusLabel[job.status]}{activeUpload?.id === job.id ? " · Current" : nextUpload?.id === job.id ? " · Next" : ""}</small>{job.error && <em>{job.error}</em>}</div></div>)}</div></aside>}
    ...
    {view === "slos" && <section className="full-page slo-page"><CurriculumPageToolbar heading={<div className="eyebrow">SESSION LEARNING OBJECTIVES</div>} filters={<><label className="sort-control"><span>Filter by</span><select value={sloWeekFilter} onChange={(event) => setSloWeekFilter(event.target.value)}><option value="all">All SLOs</option><option value="flagged">Flagged SLOs</option>{LECTURE_WEEK_OPTIONS.map((week) => <option key={week} value={week}>Week {week}</option>)}<option value="unassigned">Week unassigned</option></select></label><label className="sort-control"><span>Sort by</span><select value={sloSort} onChange={(event) => setSloSort(event.target.value as typeof sloSort)}><option value="week-asc">Week · earliest first</option><option value="name-asc">Name · A–Z</option></select></label></>} actions={<button className="export-slo-button" onClick={openSloExport}><AppIcon name="download"/>Export SLOs</button>}/><div className="slo-list">{visibleSloLectures.map((lecture) => { const expanded = expandedSloLectureIds.has(lecture.id); return <CurriculumCard key={lecture.id} title={lecture.title} course={lecture.course} lecturer={lecture.lecturer} week={lecture.week} countLabel={`${lecture.slos.length} SLOs`} primaryActionLabel={expanded ? "Hide SLOs" : "View SLOs"} onPrimaryAction={() => setExpandedSloLectureIds((current) => { const next = new Set(current); if (expanded) next.delete(lecture.id); else next.add(lecture.id); return next; })} expanded={expanded}>{expanded && <div className="expanded-slo-card"><ol>{lecture.slos.map((slo, index) => <li key={`${index}-${slo}`}><span>{index + 1}</span><p>{slo}</p><button className={lecture.flaggedSLOs.includes(index) ? "flagged" : ""} aria-label={lecture.flaggedSLOs.includes(index) ? `Unflag SLO ${index + 1}` : `Flag SLO ${index + 1}`} onClick={() => void toggleSloFlag(lecture.id, index)}><AppIcon name="flag"/></button></li>)}</ol><footer><button className="secondary-card-action" onClick={() => openSloReparse(lecture.id)}>Luna re-parse</button><button className="primary-card-action" onClick={() => openLecture(lecture)}>Open lecture</button></footer></div>}</CurriculumCard>; })}</div></section>}
