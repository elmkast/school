...
- Only after submission does Luna generate the next question, incorporating the submitted answer. It prepares while feedback remains on screen; **Next question** advances when ready.
...
- Validation enforces four/five unique choices, one valid answer index, clinical vignette length, linked reasoning steps, real source pages, and a quotation found in those pages. One bounded regeneration is allowed on validation failure.
...
The initial question normally needs two model calls (topics + question), then one per submitted answer. Failed validation can add one repair call. There is no unattended generation loop.
...
- `/ui-review` → **Quiz preview** uses the actual quiz component with deterministic fixtures, including an optional first-request failure. It exercises selection, submission, feedback, next question, retry, and exit without API calls. Fixture question content is not a demonstration of production adaptive question quality.
