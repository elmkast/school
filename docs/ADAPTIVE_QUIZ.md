...
- Validation enforces four/five unique choices, one valid answer index, clinical vignette length, linked reasoning steps, real source pages, and a quotation found in those pages. The vignette is facts only, with no question mark; the stem contains exactly one question and ends with its only question mark. Violations are regenerated automatically. Exact duplicate cases are rejected against recent and queued questions.
