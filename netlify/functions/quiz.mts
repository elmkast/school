...
const issueCodes=new Set(["QUESTION_SHAPE","FEEDBACK_FIELDS","VIGNETTE_FORMAT","VIGNETTE_LENGTH","VIGNETTE_QUESTION","STEM_QUESTION","REASONING_STEPS","CHOICES_FORMAT","DUPLICATE_CHOICES","ANSWER_INDEX","SOURCE_PAGES","SOURCE_QUOTE","SOURCE_REFERENCE","DUPLICATE_QUESTION","TOPICS_INVALID","OUTPUT_INCOMPLETE","OUTPUT_JSON"]);
  ...
  return async(request:Request)=>{
    ...
    try{
      ...
      const prompt=[
        ...
        "Use four or five plausible, mutually exclusive choices with ONE best answer. Use four unless a fifth is genuinely plausible. No all/none, letter-dependent text, or answer-length giveaways. Provide concise explanation and a rationale for each choice. The vignette must contain case facts only: never put a question, request, lead-in, or question mark in vignette. Put the entire lead-in in stem. Stem must be exactly one question and end with its only question mark. Do not ask the same thing once in vignette and again in stem. Select the sourceId of the excerpt that directly supports the tested concept; the app will attach its exact text. Never invent source IDs.",
