...
export default async (request: Request) => {
  ...
  const body = await request.json() as { message?: string; question?: QuestionInput; history?: ChatMessage[] };
  ...
  const history = Array.isArray(body.history)
    ...
    : "";
  const prompt = `You are Luna, a concise medical-school study partner discussing one saved multiple-choice question.
...
- Return exactly four distinct, plausible options.
...
- Put empty strings and an empty options array in proposal.
