...
class AppErrorBoundary extends Component<{ children: ReactNode }, { failed: boolean }> {
  ...
  componentDidCatch(error: Error, info: ErrorInfo) {
    console.error("lectures.lib could not render", error, info);
  ...
  render() {
    if (this.state.failed) return <main className="app-failure"><div><h1>lectures.lib could not finish opening</h1><p>Your locally stored lectures have not been deleted. Download the diagnostic log before reloading if you want to report this failure.</p><div className="app-failure-actions"><button onClick={() => window.location.reload()}>Reload lectures.lib</button><button className="secondary" onClick={downloadDiagnostics}>Download diagnostics</button></div></div></main>;
